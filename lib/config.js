...
export function requireProductionSecrets() {
  ...
  if (!config.opsApiKey) missing.push('OPS_API_KEY')
  if (config.mailDomain === 'tempmail.local') {
    missing.push('NEXT_PUBLIC_MAIL_DOMAIN')
  }
  if (config.allowedOrigins.includes('*')) {
    missing.push('ALLOWED_ORIGINS_WITHOUT_WILDCARD')
  }
  if (config.demoInboundEnabled) {
    missing.push('DEMO_INBOUND_ENABLED_FALSE')
  }
  if (!config.hasRedis) {
